...
const auth = require('./Meta Data/auth.json')
const config = require('./Meta Data/Config.json')
//Bring's discord.js, Starts up a dicord client for the bot and saves the token as well as brings up my config file
const calculator = require('./Commands/Bot_Calculator.js')
const numberGame = require('./Commands/Bot_Number_game.js')
const campfire = require('./Commands/Bot_Campfire.js')
const help = require('./Commands/Bot_Help.js')
...
client.on('message', function(msg) {
  ...
  if (command === 'ping') {
    try {
      ...
      console.log(`${msg.author.tag} encountered an error when attempting to ping ${client.user.tag}`);
    }
  }
  ...
  if (command === 'calc') {
    try {
      ...
      msg.channel.send(`beep boop something went wrong with this command beep boop`)
      console.log(`${msg.author.tag} encountered an error when attempting to make a calculation`);
    }
  }
  ...
  if (command === 'numgame') {
    try {
      ...
      msg.channel.send(`beep boop something went wrong with this command beep boop`)
      console.log(`${msg.author.tag} encountered an error when attempting to play the number game`)
      console.log(`${msg.content}${numberGame.randomNumber}`)
    }
  }
  ...
  //If the command is campfire a rich embed will be sent with a nice message and a gif of a campfire and it will be logged otherwise an error will be logged

  if (command === 'help') {
    try {
      msg.reply(help())
      console.log(`${msg.author.tag} has taken a look at help`)
    } catch (error) {
      msg.channel.send(`beep boop something went wrong with this command beep boop`)
      console.log(`${msg.author.tag} encountered an error when attempting to get help. How ironic`)
    }
  }
