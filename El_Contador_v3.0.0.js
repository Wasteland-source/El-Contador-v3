...
client.on('message', function(msg) {
  ...
  if (command === 'numgame') {
    try {
      msg.channel.send(numberGame.prompt(msg.content))
  ...
  if (command === 'campfire') {
    try {
      const campfireGIF = new Discord.Attachment('https://media.giphy.com/media/vlEyXRgGUjSoM/giphy.gif');

      msg.channel.send('You sit down at the top of a small hill at the end of a long day with a small group of your closest friends, as you and your friends sit around the fire, talking late into the night you look deep into the fire for a moment and smile to yourself. Everything is going to be okay')
      msg.channel.send(campfireGIF)
    } catch (error) {
      msg.channel.send(`I'm sorry ${msg.author.username}, I can't do that ${msg.author.username}. Beep boop`)
